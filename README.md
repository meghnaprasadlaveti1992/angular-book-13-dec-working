# AngularJS Book Catalog Tutorial Application


## Overview

This application takes the developer through the process of building a web-application using
AngularJS. The application is loosely based on the **Google Book Gallery**, which no longer exists.
Here is a historical reference: [Google Book Gallery on WayBack][google-book-gallery]
...
- Add a static list with two books.

### step-2 _Angular Templates_

- Convert the static book list to dynamic by:
  - Creating a `BookListController` controller.
...
- Add a simple unit test for the `BookListController` controller to show how to write tests and
  run them using Karma.

### step-3 _Components_

- Introduce components.
- Combine the controller and the template into a reusable, isolated `bookList` component.
- Refactor the application and tests to use the `bookList` component.
...
- Add a search box to demonstrate:
  ...
  - How `ngRepeat` automatically shrinks and grows the number of books in the view.
...
- Add an `age` property to the book model.
- Add a drop-down menu to control the book list order.
...
- Replace the in-memory dataset with data loaded from the server (in the form of a static
  'book.json' file to keep the tutorial backend agnostic):
...
- Add a book image and links to book pages.
- Add an end-to-end test that verifies the book links.
...
- Create a book list route (`/books`):
  - Map `/books` to the existing `bookList` component.
- Create a book detail route (`/books/:bookId`):
  - Map `/books/:bookId` to a new `bookDetail` component.
  - Create a dummy `bookDetail` component, which displays the selected book ID.
  - Pass the `bookId` parameter to the component's controller via `$routeParams`.

### step-10 _More Templating_

- Implement fetching data for the selected book and rendering to the view:
  - Use `$http` in `BookDetailController` to fetch the book details from a JSON file.
  - Create the template for the detail view.
- Add CSS styles to make the book detail page look "pretty-ish".

### step-11 _Custom Filters_

- Implement a custom `checkmark` filter.
- Update the `bookDetail` template to use the `checkmark` filter.
- Add a unit test for the `checkmark` filter.

### step-12 _Event Handlers_

- Make the thumbnail images in the book detail view clickable:
  - Introduce a `mainImageUrl` property on `BookDetailController`.
...
- Create a custom `Book` service that represents the RESTful client.
- Use a custom Jasmine equality tester in unit tests to ignore irrelevant properties.

### step-14 _Animations_

- Add animations to the application:
  - Animate changes to the book list, adding, removing and reordering books with `ngRepeat`.
  - Animate view transitions with `ngView`.
  - Animate changes to the main book image in the book detail view.
...
## Development with `angular-bookcat`
...
app/                     --> all the source code of the app (along with unit tests)
  ...
  core/                  --> all the source code of the core module (stuff used throughout the app)
    ...
    book/...            --> files for the `core.book` submodule, including JS source code, specs
    core.module.js       --> the core module
  img/...                --> image files
  book-detail/...       --> files for the `bookDetail` module, including JS source code, HTML templates, specs
  book-list/...         --> files for the `bookList` module, including JS source code, HTML templates, specs
  books/...             --> static JSON files with book data (used to fake a backend API)
...
[google-book-gallery]: http://web.archive.org/web/20131215082038/http://www.android.com/devices
